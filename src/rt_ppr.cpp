...
int childPprData::GetDimmSerialNum(uint16_t Socket, uint16_t Ch, uint16_t Chip)
{
    ...
    dimmAddr = (uint8_t)(Ch & DIMM_SN_CH_MASK) +
               (uint8_t)((Chip & DIMM_SN_CHIP_MASK) << DIMM_SN_CHIP_SHIFT);

    if (Socket == DIMM_SOCKET_0)
    { // Socket 0
        if (Chip < DIMM_CHIP_2DPC)
        { // 1 DPC
            dimm = Ch + (Chip * (MAX_DIMM_SLOT / 4));
        }
        else
        { // 2 DPC
    ...
    { // socket 1
        dimm = ((MAX_DIMM_SLOT / 2) + Ch + (Chip * (MAX_DIMM_SLOT / 4)));
    }
